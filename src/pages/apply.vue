    ...
    <section class="flex screen-size flex-col">
        ...
        <div class="flex flex-grow justify-center px-6">
            <div class="w-[850px] max-w-full mt-20 ">
                <OnboardingQuestion v-model="answers.username" v-if="step == 1" :question="questions[0]" :step :total @next="next" @back="back"></OnboardingQuestion>
                <OnboardingQuestion v-model="answers.based_in" v-if="step == 2" :question="questions[1]" :step :total @next="next" @back="back"></OnboardingQuestion>
                <OnboardingQuestion v-model="answers.birthdate"  v-if="step == 3" :question="questions[2]" :step :total @next="next" @back="back"></OnboardingQuestion>
                <OnboardingQuestion v-model="answers.project_types"  v-if="step == 4" :question="questions[3]" :step :total @next="next" @back="back"></OnboardingQuestion>
                <OnboardingQuestion v-model="answers.additional_info" v-if="step == 5" :question="questions[4]" :step :total @next="next" @back="back"></OnboardingQuestion>
                <OnboardingQuestion v-model="answers.tiktok" v-if="step == 6" :question="questions[5]" :step :total @next="next" @back="back"></OnboardingQuestion>
                <button v-if="step == total" @click="finished" class="bg-black text-white px-24 py-3 rounded-lg mt-6">finish</button>
            </div>
        </div>
    </section>
</template>
