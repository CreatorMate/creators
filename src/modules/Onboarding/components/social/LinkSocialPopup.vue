<script setup lang="ts">
	...
	async function verifyNewName() {
		const igProfile: APIResponse<InstagramVerification> = await API.ask(
			`/onboarding/verification?handle=${localValue.value}`,
		);
...
<template>
	<div class="bg-white p-6 rounded-lg shadow-xl w-[600px] relative">
		<div class="w-full mb-4 flex justify-between">
			<h2 v-if="!verified" class="text-lg font-semibold">
				link your instagram account
			</h2>
			<h2 v-if="verified && !verified.verified" class="text-lg font-semibold">
				dm this code to @trycreatormate on instagram
			</h2>
			<h2 v-if="verified?.verified" class="text-lg font-semibold">
				your linked instagram accounts
			</h2>
		...
		<InsertInstagramName
			v-if="!verified"
			@confirm-name="verifyNewName"
			v-model="localValue"
		/>
		<div v-else-if="verified.verified">
			<p v-if="verified?.verified" class="mb-4 text-[#3C3C3C]">
				manage your connected ig account
			</p>
			<div class="flex items-center w-fit gap-2 px-6 py-2 bg-[#F1F1F1]">
				<p>{{ verified.handle }}</p>
				<Icon
					@click="deleteAccount"
					class="cursor-pointer hover:text-black text-gray-800"
		...
		<VerifyInstagramAccount
			v-else
			@confirm-name="emit('confirm', localValue)"
			@cancel="deleteAccount"
			v-model="localValue"
			:verified
		/>
	</div>
