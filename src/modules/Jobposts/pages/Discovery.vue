...
<template>
    ...
    <section class="flex flex-col px-4 gap-3">
        ...
        <div class="w-full flex gap-1 text-size-XS">
            <div @click="switchTab(DiscoveryFilters.FOR_YOU)" class=" text-center border rounded-full py-3 px-5 w-full cursor-pointer"
                 :class="{'border-none bg-black text-white': active == DiscoveryFilters.FOR_YOU}">for you
            </div>
            <div @click="switchTab(DiscoveryFilters.ALL_JOBS)" class=" text-center border rounded-full py-3 px-5 w-full cursor-pointer"
                 :class="{'border-none bg-black text-white': active == DiscoveryFilters.ALL_JOBS}">all jobs
            </div>
            <div @click="switchTab(DiscoveryFilters.ARCHIVED)" class=" text-center border rounded-full py-3 px-5 w-full cursor-pointer"
                 :class="{'border-none bg-black text-white': active == DiscoveryFilters.ARCHIVED}">archived
            </div>
            <div @click="showFilters = !showFilters" class=" text-center rounded-full py-3 px-5 filter-button cursor-pointer border bg-[#F8F8F8] flex gap-2.5 items-center w-full justify-center"
        ...
        <div class="relative w-full">
            <div
                ...
                class="text-size-XS text-[#111111] bg-white py-3 px-5 rounded-xl flex flex-col gap-2 absolute top-0 w-full z-50">
                <p class="">roles</p>
