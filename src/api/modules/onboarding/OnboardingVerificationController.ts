...
import {
	...
	inheritResponse,
	successResponse,
} from "~/src/api/utils/HonoResponses";
	...
	async endpoints() {
			...
			async (context: Context): Promise<any> => {
				...
				if (!handle) {
					const getVerification =
						await this.getPrisma().instagram_account.findFirst({
				...
				if (!verification)
					return errorResponse(
						context,
						"something went wrong generating verification code.",
					);
		...
		this.app.delete(
			...
			async (context: Context): Promise<any> => {
				...
				const user = this.getHonoUser(context);
				const getVerification =
					await this.getPrisma().instagram_account.deleteMany({
						where: { user_id: user.id, handle: handle },
					});
				return successResponse(context, getVerification);
			},
			...
			if (senderId && text && text.includes("verify")) {
				...
				if (!response.ok) {
					...
					await this.sendMessage(
						senderId,
						"Instagram verification service down try again later",
					);
					return null;
				}

				const data: { username: string; follower_count: number } =
					await response.json();
				...
				if (!check) {
					await this.sendMessage(
						senderId,
						"Error: Username does not match the associated Instagram account",
					);
