...
onMounted(async () => {
  ...
  const response: APIResponse = await $fetch(
    `/API/phyllo?creatorId=${accountState.user.id}&creatorEmail=${accountState.user.email}`,
  );

  if (!response.success) {
    console.error(
      "could not initiate a connection with instagram through phyllo",
    );
    return;
  }
  ...
  phylloConnect.on(
    "accountConnected",
    async (accountId: any, workplatformId: any, userId: any) => {
      ...
      const result: APIResponse = await $fetch(
        `${appSettings.baseUrl}/API/creators/${userId}/accounts`,
        {
          method: "POST",
          body: JSON.stringify({
            accountId: accountId,
  ...
  phylloConnect.on(
    ...
    async (accountId: any, workplatformId: any, userId: any) => {
      ...
      const result: APIResponse = await $fetch(
        `${appSettings.baseUrl}/API/creators/${userId}/accounts/${accountId}`,
        {
          method: "DELETE",
        },
      );
  ...
  phylloConnect.on(
    "connectionFailure",
    (reason: any, workplatformId: any, userId: any) => {},
  );
